...
CalendarPlugin.ValidatableField = Ember.View.extend
  ...
  templateBinding:    SC.Binding.oneWay('controlGroup').transform (controlGroup) ->
    if controlGroup then CONTROL_GROUP_TEMPLATE else BASIC_FIELD_TEMPLATE

  init: ->
    @_super()
    @set('validator', CalendarPlugin.Validator.create({host: this}))
    options = {}
